...
with open("full keywords.txt") as f:
        codes = f.read().splitlines()
codes = [sub(r"[^a-z0-9\?]", "", code.lower()) for code in codes]
...
def process_html(html_string: str) -> str:
        ...
        for i, (image_format, base64_img) in enumerate(matches, start=1):
                ...
                new_html = sub(
                        f"data:image/{image_format};base64,{escape(base64_img)}",
                        replacement_text.replace("\\", r"\\"),
                        new_html,
                        count=1,
                )
...
Content-Disposition: form-data; name="code"\r
\r
{code}\r
--kljmyvW1ndjXaOEAg4vPm6RBUqO6MC5A--\r
"""
        unique_responses: set[str | bytes] = set()
        ...
        while attempts < max_attempts:
                ...
                if "text/html" not in content_type:
                        ...
                        message = f"{str(attempts).zfill(3)} - New unique response ({len(unique_responses)}): file saved as `{file_path}`"  # noqa: E501
                        print(message)
                        webhook.content = message
                        _ = webhook.execute()
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                        _ = f.write(
                                html_template.replace("{{NEW_BODY}}", response)
                                .replace("{{CODE}}", code)
                                .replace("hidden", "")
                        )

                chunked_response = (
                        [response[i : i + 2000 - 10] for i in range(0, len(response), 2000 - 10)]
                        if len(response) > 2000 - 10
                        else [response]
                )
